...
final class CameraXProviderCoordinator {
    ...
    static synchronized <T> T withSingleOwner(
        ...
        if (activeProvider != provider || primaryOwner != owner || secondaryOwner != null) {
            if (NebulaCameraLog.DEBUG) NebulaCameraLog.log("CXOwner single acquire owner=" + ownerName(owner)
    ...
    static synchronized boolean withConcurrentOwners(
        ...
        if (NebulaCameraLog.DEBUG) NebulaCameraLog.log("CXOwner concurrent acquire first=" + ownerName(first)
    ...
    private static void resetActiveGraphLocked() {
        ...
        if (NebulaCameraLog.DEBUG) NebulaCameraLog.log("CXOwner reset generation=" + invalidationGeneration
        ...
        if (provider != null) {
            try {
                provider.unbindAll();
                if (NebulaCameraLog.DEBUG) NebulaCameraLog.log("CXOwner unbindAll dispatched generation="
                        + invalidationGeneration);
            } catch (Throwable error) {
                FileLog.e("CameraX provider reset failed", error);
                if (NebulaCameraLog.DEBUG) NebulaCameraLog.log("CXOwner unbindAll FAILED generation="
