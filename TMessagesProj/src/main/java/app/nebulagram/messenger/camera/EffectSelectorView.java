...
public class EffectSelectorView extends LinearLayout {
    ...
    public void resetSelectedEffect() {
        for (int i = 0; i < getChildCount(); i++) {
            ...
            buttonEffect.toggleButton(buttonEffect.cameraType == NebulaCameraXController.CAMERA_NONE, false);
            if (buttonEffect.cameraType == NebulaCameraXController.CAMERA_NONE) {
    ...
    public void loadEffects(BaseCameraView baseCameraView) {
        if (getChildCount() == 0) {
            ...
            if (CameraXUtils.isCurrentCameraCameraX() && baseCameraView instanceof NebulaCameraXView cameraXView) {
                if (!fuckup && cameraXView.isNightModeSupported()) {
                    list_effect.add(NebulaCameraXController.CAMERA_NIGHT);
                }
                if (!fuckup && cameraXView.isAutoModeSupported()) {
                    list_effect.add(NebulaCameraXController.CAMERA_AUTO);
                }
                list_effect.add(NebulaCameraXController.CAMERA_NONE);
                if (cameraXView.isWideModeSupported()) {
                    list_effect.add(NebulaCameraXController.CAMERA_WIDE);
                }
                if (!fuckup && cameraXView.isHdrModeSupported()) {
                    list_effect.add(NebulaCameraXController.CAMERA_HDR);
                }
            ...
            for (int i = 0; i < list_effect.size(); i++) {
                ...
                buttonEffect.toggleButton(effect == NebulaCameraXController.CAMERA_NONE, false);
                if (effect == NebulaCameraXController.CAMERA_NONE) {
                    oldSelection = buttonEffect;
                }
