...
public class NebulaVideoMessagesHelper {

    private static final String TAG = "NebulaVideoMessagesHelper";

    public NebulaCameraXController cameraXController;
    private volatile NebulaCameraXSurfaceSession cameraXSession;
    private volatile NebulaCameraXSurfaceSession cameraXSecondarySession;
    ...
    private final ArrayList<NebulaCameraXSurfaceSession> closingCameraXSessions =
            new ArrayList<>(2);
    ...
    public void createCameraX(InstantCameraView instantCameraView, final SurfaceTexture... surfaceTextures) {
        if (instantCameraView == null || surfaceTextures == null || surfaceTextures.length == 0
            ...
            if (NebulaCameraLog.DEBUG) NebulaCameraLog.log("RoundCX create rejected view=" + (instantCameraView != null)
        ...
        if (NebulaCameraLog.DEBUG) NebulaCameraLog.log("RoundCX create surfaces=" + surfaceTextures.length
    ...
    private void createCameraXAttempt(InstantCameraView instantCameraView,
        ...
        if (NebulaCameraLog.DEBUG) NebulaCameraLog.log("RoundCX attempt generation=" + currentGeneration
    ...
    private void startCameraX(InstantCameraView instantCameraView,
        ...
        if (currentGeneration != generation || requestedSurface == null) {
            if (NebulaCameraLog.DEBUG) NebulaCameraLog.log("RoundCX start stale/rejected generation=" + currentGeneration
        ...
        int size = NebulaConfig.getVideoMessagesResolutionPx(512);
        ...
        if (NebulaCameraLog.DEBUG) NebulaCameraLog.log("RoundCX start generation=" + currentGeneration
                + " front=" + requestedFrontFacing + " dual=" + dual
                + " compatibility=" + compatibilityMode
                + " startWide=" + NebulaConfig.startFromUltraWideCam
        ...
        cameraXSession = new NebulaCameraXSurfaceSession(
                instantCameraView.getContext(), surfaceTexture, frontFacing,
                targetWidth, targetHeight,
                false, dual, false, true,
                new NebulaCameraXSurfaceSession.Callback() {
                    @Override
                    public void onReady(int width, int height) {
                        if (NebulaCameraLog.DEBUG) NebulaCameraLog.log("RoundCX primary ready generation="
                    ...
                    public void onFailure(Throwable error) {
                        ...
                        FileLog.e("Round video CameraX bind failed", error);
                            if (NebulaCameraLog.DEBUG) NebulaCameraLog.log("RoundCX primary FAILED generation="
        ...
        if (dual) {
            cameraXSecondarySession = new NebulaCameraXSurfaceSession(
                    ...
                    new NebulaCameraXSurfaceSession.Callback() {
                        @Override
                        public void onReady(int width, int height) {
                            if (NebulaCameraLog.DEBUG) NebulaCameraLog.log("RoundCX secondary ready generation="
                                    + currentGeneration + " size=" + width + "x" + height);
                        ...
                        public void onFailure(Throwable error) {
                            ...
                            if (NebulaCameraLog.DEBUG) NebulaCameraLog.log("RoundCX secondary FAILED generation="
                                    + currentGeneration, error);
                            handleCameraXDualFailure(
                                    instantCameraView, currentGeneration,
                                    isConcurrentResourceFailure(error));
    ...
    private boolean tryBindConcurrentRoundCamera(InstantCameraView view, int width, int height) {
        ...
        if (NebulaCameraLog.DEBUG) NebulaCameraLog.log("RoundCX concurrent probe generation=" + generation
        ...
        if (!cameraXSession.isConcurrentWith(cameraXSecondarySession)) {
            if (cameraXDualBindPending) {
                if (NebulaCameraLog.DEBUG) NebulaCameraLog.log("RoundCX concurrent probe ignored while bind pending generation="
                        + generation);
                return false;
            }
            ...
            if (!bound) {
                ...
                if (NebulaCameraLog.DEBUG) NebulaCameraLog.log("RoundCX concurrent bind returned false generation="
                        + generation);
                return false;
            }
        }
        if (NebulaCameraLog.DEBUG) NebulaCameraLog.log("RoundCX concurrent ready generation=" + generation);
        return true;
    }
    ...
    private boolean scheduleCameraXDualBusyRetry(InstantCameraView view) {
        ...
        if (NebulaCameraLog.DEBUG) NebulaCameraLog.log("RoundCX provider-busy retry scheduled generation="
        ...
        AndroidUtilities.runOnUIThread(() -> {
            ...
            cameraXDualBusyRetryPending = false;
            cameraXDualTransitionPending = false;
            if (NebulaCameraLog.DEBUG) NebulaCameraLog.log("RoundCX provider-busy retry executing generation="
                    + expectedGeneration + " retry=" + retry);
    ...
    private void handleCameraXDualFailure(InstantCameraView view,
        ...
        if (NebulaCameraLog.DEBUG) NebulaCameraLog.log("RoundCX dual failure generation=" + expectedGeneration
                + " compatibility=" + cameraXDualCompatibilityMode
    ...
    private static boolean isConcurrentResourceFailure(Throwable error) {
        ...
        while (current != null) {
            if (current instanceof
                    NebulaCameraXController.ConcurrentCameraResourceException) {
                return true;
            }
    ...
    public boolean retryCameraXDual(InstantCameraView view) {
        ...
        if (NebulaCameraLog.DEBUG) NebulaCameraLog.log("RoundCX dual compatibility retry generation=" + generation);
    ...
    private void collapseCameraXDualToSingle(InstantCameraView view,
        ...
        if (NebulaCameraLog.DEBUG) NebulaCameraLog.log("RoundCX dual collapse-to-single generation="
                + expectedGeneration);
        cameraXDualTransitionPending = true;
        cameraXDualMode = false;
        cameraXDualCompatibilityMode = false;
        final NebulaCameraXSurfaceSession primary = cameraXSession;
        NebulaCameraXSurfaceSession secondary = cameraXSecondarySession;
            ...
            AndroidUtilities.runOnUIThread(() -> {
                ...
                Runnable rebindSingle = () -> {
                    ...
                    if (primary.rebindSingle()) {
                        ...
                        if (NebulaCameraLog.DEBUG) NebulaCameraLog.log("RoundCX single fallback rebound generation="
                                + expectedGeneration);
                    } else {
                        FileLog.e("Round video CameraX single fallback bind failed");
                        if (NebulaCameraLog.DEBUG) NebulaCameraLog.log("RoundCX single fallback FAILED generation="
                                + expectedGeneration);
                    }
                };
    ...
    public NebulaCameraXSurfaceSession getCurrentSession() {
    ...
    public NebulaCameraXSurfaceSession getRearSession() {
    ...
    public int getSessionIndex(NebulaCameraXSurfaceSession session) {
    ...
    public void switchCameraX(InstantCameraView instantCameraView) {
        ...
        if (NebulaCameraLog.DEBUG) NebulaCameraLog.log("RoundCX switch requested generation=" + generation
                + " front=" + frontFacing + " dualSessions="
                + (cameraXSecondarySession != null));
        if (cameraXSecondarySession != null && cameraXSession != null) {
            if (cameraXSession.isConcurrentWith(cameraXSecondarySession)) {
                NebulaCameraXSurfaceSession current = getCurrentSession();
                ...
                if (NebulaCameraLog.DEBUG) NebulaCameraLog.log("RoundCX dual switch ignored: pair not ready generation="
                        + generation);
            }
            return;
        }
        if (cameraXSession != null) {
            ...
            final NebulaCameraXSurfaceSession switchingSession = cameraXSession;
    ...
    public void destroyCameraX(InstantCameraView instantCameraView, Runnable onClosed) {
        ...
        generation++;
        if (NebulaCameraLog.DEBUG) NebulaCameraLog.log("RoundCX destroy generation=" + oldGeneration
                + " next=" + generation);
        cameraXDualTransitionPending = false;
        cameraXDualCompatibilityMode = false;
        cameraXDualBindPending = false;
    ...
    private void closeActiveCameraXSessions(Runnable onClosed) {
        ArrayList<NebulaCameraXSurfaceSession> sessions = new ArrayList<>(2);
        ...
        if (NebulaCameraLog.DEBUG) NebulaCameraLog.log("RoundCX close sessions count=" + sessions.size()
                + " pending=" + closingCameraXSessions.size());
        for (NebulaCameraXSurfaceSession session : sessions) {
            try {
    ...
    private void retireCameraXSession(NebulaCameraXSurfaceSession session) {
    ...
    private void addClosingSessionLocked(
            ArrayList<NebulaCameraXSurfaceSession> sessions,
            NebulaCameraXSurfaceSession session) {
    ...
    private void onCameraXSessionClosed(NebulaCameraXSurfaceSession session) {
    ...
    private static void applyCameraXFlash(NebulaCameraXSurfaceSession session,
                                          NebulaCameraXSurfaceSession current,
                                          boolean enableRear) {
        if (session != null) {
