...
final class CameraXZoomCoordinator {
    ...
    void requestZoomRatio(float ratio) {
        ...
        if (targetCamera == null || !isFinite(ratio)) {
            if (NebulaCameraLog.DEBUG) NebulaCameraLog.log("CXZoom request ignored camera="
        ...
        try {
            ...
            if (state == null) {
                if (NebulaCameraLog.DEBUG) NebulaCameraLog.log("CXZoom request ignored: state unavailable ratio="
                        + ratio);
                return;
            }
            requestedRatio = clamp(ratio,
                    state.getMinZoomRatio(), state.getMaxZoomRatio());
            if (NebulaCameraLog.DEBUG) NebulaCameraLog.log("CXZoom request ratio=" + ratio
    ...
    private void dispatchLatest() {
        ...
        try {
            ...
            if (NebulaCameraLog.DEBUG) NebulaCameraLog.log("CXZoom submit ratio=" + targetRatio
        ...
        future.addListener(() -> {
            ...
            try {
                future.get();
                if (NebulaCameraLog.DEBUG) NebulaCameraLog.log("CXZoom applied ratio=" + submittedRatio
                        + " generation=" + generation);
            } catch (Throwable error) {
