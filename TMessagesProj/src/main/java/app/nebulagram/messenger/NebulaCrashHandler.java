...
public final class NebulaCrashHandler {
            ...
            if (trace != null) {
                for (StackTraceElement el : trace) {
                    ...
                    if (cls.startsWith("com.chaquo.python.")
                            || cls.startsWith("app.nebulagram.messenger.plugins.xposed.")
                            || cls.startsWith("app.nebulagram.messenger.plugins.hooks.")
                            || cls.startsWith("app.nebulagram.messenger.plugins.intents.")) {
                        return true;
                    }
    ...
    public static void dump(Thread thread, Throwable t) {
        ...
        try {
            ...
            try (PrintWriter pw = new PrintWriter(new BufferedWriter(new FileWriter(f)))) {
                ...
                try {
                    pw.println("App: " + NebulaConfig.APP_NAME + " " + NebulaConfig.VERSION_NAME
                            + " (code " + versionCode() + ")");
                } catch (Throwable ignored) {
                ...
                if (oom) {
                    try {
                        ...
                        pw.println("Heap dump: nebula-logs/oom-" + reportId
            ...
            if (oom) {
                try {
                    ...
                    android.util.Log.e("nebula-crash", "hprof dumped: " + hp.getAbsolutePath());
                } catch (Throwable ignored) {}
            }
            android.util.Log.e("nebula-crash", "report written to " + f.getAbsolutePath());
            
            if (!oom) {
                String preview = readReportPreview(f, 64 * 1024);
                if (preview != null) {
                    android.util.Log.e("nebula-crash", preview);
                    copyToClipboard(preview);
                }
            }
        } catch (Throwable ignored) {
            try {
                android.util.Log.e("nebula-crash", "failed to dump", ignored);
    ...
    private static String readReportPreview(File file, int maxChars) {
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(file))) {
            ...
            if (reader.read() >= 0) {
                result.append("\n[report preview truncated; full report is in nebula-logs]");
            }
    ...
    private static void copyToClipboard(String text) {
        try {
            ...
            if (cm != null) {
                cm.setPrimaryClip(android.content.ClipData.newPlainText("NebulaGram crash", text));
            }
        } catch (Throwable ignored) {}
    }
    ...
    public static File getLogDir() {
        ...
        if (context != null) {
            try {
                ...
                if (ext != null) {
                    File dir = ensureLogDir(new File(ext, "nebula-logs"));
            ...
            try {
                ...
                if (files != null) {
                    File dir = ensureLogDir(new File(files, "nebula-logs"));
                    if (dir != null) return dir;
                }
            } catch (Throwable ignored) {
            }
        ...
        try {
            File legacy = ensureLogDir(
                    new File(Environment.getExternalStorageDirectory(), "NebulaGram/logs"));
