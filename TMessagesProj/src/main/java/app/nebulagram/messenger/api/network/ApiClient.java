package app.nebulagram.messenger.api.network;

import app.nebulagram.messenger.api.dto.BadgeDTO;
import app.nebulagram.messenger.api.dto.ProfileDTO;
...
public final class ApiClient {
    ...
    private static final String NEBULA_BASE_URL = org.telegram.messenger.BuildConfig.NEBULA_API_BASE_URL;
    ...
    public static Map<Long, BadgeDTO> getNebulaBadges() {
        BadgeFetchResult result = fetchNebulaBadges();
        return result.success ? result.badges : Collections.emptyMap();
    }

    public static BadgeFetchResult fetchNebulaBadges() {
        if (NEBULA_BASE_URL == null || NEBULA_BASE_URL.trim().isEmpty()) {
            return new BadgeFetchResult(false, Collections.emptyMap());
        }
        Request req = new Request.Builder().url(NEBULA_BASE_URL + "badges").get().build();
        try (Response resp = CLIENT.newCall(req).execute()) {
            ...
            FileLog.e("nebula-badges: getNebulaBadges failed", e);
            return new BadgeFetchResult(false, Collections.emptyMap());
        } catch (Throwable t) {
            FileLog.e("nebula-badges: getNebulaBadges parse failed", t);
            return new BadgeFetchResult(false, Collections.emptyMap());
        }
    }
}
