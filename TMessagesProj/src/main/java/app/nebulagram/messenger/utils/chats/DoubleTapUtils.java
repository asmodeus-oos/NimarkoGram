...
public final class DoubleTapUtils {
    ...
    public static boolean isCustomActionEnabled() {
        int a = NebulaConfig.doubleTapAction;
        return a != NebulaConfig.DTAP_NONE && a != NebulaConfig.DTAP_REACTION;
    }

    public static boolean isDoubleTapEnabled() {
        return NebulaConfig.doubleTapAction != NebulaConfig.DTAP_NONE;
    }

    public static boolean canHandle(MessageObject message) {
        if (message == null) return false;
        int action = NebulaConfig.doubleTapAction;
        switch (action) {
            case NebulaConfig.DTAP_REPLY:
                return !message.isSending() && message.getId() > 0 && !message.isSponsored();
            case NebulaConfig.DTAP_SAVE:
                return !message.isSending() && !message.isSponsored() && !message.isSecret();
            case NebulaConfig.DTAP_EDIT:
                return message.canEditMessage(null);
            case NebulaConfig.DTAP_EDIT_OR_REACTION:
            ...
            case NebulaConfig.DTAP_TRANSLATE:
        ...
        try {
            switch (NebulaConfig.doubleTapAction) {
                ...
                case NebulaConfig.DTAP_SAVE: {
                    ...
                    long targetId = NebulaConfig.getEffectiveSavedMessagesDialogId(message.currentAccount, selfId);
