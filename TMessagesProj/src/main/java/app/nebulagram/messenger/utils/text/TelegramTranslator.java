 ...
 */
package app.nebulagram.messenger.utils.text;
...
import app.nebulagram.messenger.utils.chats.NebulaMessageHelper;
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (q instanceof TLRPC.TL_textWithEntities || q instanceof CharSequence) {
                ...
                if (translationData instanceof String || translationData instanceof TLRPC.TL_textWithEntities) {
                    ...
                    if (res.additionalInfo instanceof NebulaMessageHelper.ReplyMarkupButtonsTexts) {
                        NebulaMessageHelper.ReplyMarkupButtonsTexts buttonRows = (NebulaMessageHelper.ReplyMarkupButtonsTexts) res.additionalInfo;
                        for (int x = 0; x < buttonRows.getTexts().size(); x++) {
                            textMessagesQuery.addAll(buttonRows.getTexts().get(x));
                        }
                    }
                } else if (translationData instanceof NebulaMessageHelper.PollTexts) {
                    textMessagesQuery.addAll(new ArrayList<>(((NebulaMessageHelper.PollTexts) translationData).getTexts()));
                }
            }
        }
        ...
        for (int i = 0; i < translations.size(); i++) {
            ...
            if (q instanceof TLRPC.TL_textWithEntities || q instanceof CharSequence) {
                ...
                if (translationData instanceof String || translationData instanceof TLRPC.TL_textWithEntities) {
                    ...
                    NebulaMessageHelper.PollTexts pollTexts = (NebulaMessageHelper.PollTexts) translationData;
