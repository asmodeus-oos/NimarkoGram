...
package app.nebulagram.messenger.utils.folders;
...
public final class NebulaFoldersHelper {

    private NebulaFoldersHelper() {
    }
    ...
    public static int getFolderColor(int folderId, int defaultColor) {
        Map<Integer, Integer> colors = NebulaConfig.folderColors;
    ...
    public static void setFolderColor(int folderId, int color) {
        if (NebulaConfig.folderColors == null) {
            NebulaConfig.folderColors = new HashMap<>();
        }
        if (color == COLOR_UNSET) {
            NebulaConfig.folderColors.remove(folderId);
        } else {
            NebulaConfig.folderColors.put(folderId, color);
        }
        NebulaConfig.saveFolderColors();
    }
    ...
    public static int getFolderBadgeMode(int folderId) {
        Map<Integer, Integer> modes = NebulaConfig.folderBadgeMode;
        if (modes == null) return NebulaConfig.FOLDER_BADGE_NUMBER;
        Integer v = modes.get(folderId);
        if (v == null) return NebulaConfig.FOLDER_BADGE_NUMBER;
        
        if (v < NebulaConfig.FOLDER_BADGE_NUMBER || v > NebulaConfig.FOLDER_BADGE_HIDDEN) {
            return NebulaConfig.FOLDER_BADGE_NUMBER;
        }
        return v;
    }

    public static void setFolderBadgeMode(int folderId, int mode) {
        if (NebulaConfig.folderBadgeMode == null) {
            NebulaConfig.folderBadgeMode = new HashMap<>();
        }
        if (mode == NebulaConfig.FOLDER_BADGE_NUMBER) {
            
            NebulaConfig.folderBadgeMode.remove(folderId);
        } else {
            NebulaConfig.folderBadgeMode.put(folderId, mode);
        }
        NebulaConfig.saveFolderBadgeMode();
    }

    public static int filterTabCounter(int folderId, int rawCount) {
        if (NebulaConfig.tabsNoUnread) return 0;
        int mode = getFolderBadgeMode(folderId);
        if (mode == NebulaConfig.FOLDER_BADGE_HIDDEN) return 0;
        if (mode == NebulaConfig.FOLDER_BADGE_DOT) return 0; 
        return rawCount;
    }

    public static boolean shouldShowDot(int folderId, int rawCount) {
        if (NebulaConfig.tabsNoUnread) return false;
        if (rawCount <= 0) return false;
        return getFolderBadgeMode(folderId) == NebulaConfig.FOLDER_BADGE_DOT;
    }

    public static int nextFolderId(int accountNum, int currentId, int direction) {
        if (!NebulaConfig.folderSwipeEnabled) return currentId;
    ...
    public static Map<String, List<Integer>> getFolderGroups() {
        String json = NebulaConfig.folderGroupsJson;
    ...
    public static void setFolderGroups(Map<String, List<Integer>> groups) {
        NebulaConfig.setFolderGroupsJson(
