...
public class NebulaProfileActivityHelper extends BaseController {

    private static final NebulaProfileActivityHelper[] Instance = new NebulaProfileActivityHelper[UserConfig.MAX_ACCOUNT_COUNT];
    ...
    public NebulaProfileActivityHelper(int num) {
        super(num);
    }

    public static synchronized NebulaProfileActivityHelper getInstance(int num) {
        NebulaProfileActivityHelper localInstance = Instance[num];
        if (localInstance == null) {
            Instance[num] = localInstance = new NebulaProfileActivityHelper(num);
        }
    ...
    public void injectProfileMenu(
        ...
        if (user != null) {
            ...
            if (!UserObject.isUserSelf(user) && currentEncryptedChat == null && !isBot) {
                if (emojiDocumentId != 0 && NebulaConfig.profileBackgroundEmoji) {
                ...
                if (emojiDocumentId != 0
                        ...
                        && NebulaConfig.profileBackgroundEmoji) {
                    otherItem.addSubItem(
    ...
    public void showUserInfo(BaseFragment baseFragment, long userID) {
        ...
        sb.append('\n').append(NebulaExtra.getProfileDC(currentAccount, user, null));

        NebulaExtra.RegistrationInfo regInfo = NebulaExtra.resolveRegistrationInfo(baseFragment, userID);
