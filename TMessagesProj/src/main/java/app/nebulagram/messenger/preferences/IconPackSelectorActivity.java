...
public class IconPackSelectorActivity extends BaseFragment {
    ...
    private static final int[] VALUES = {
            NebulaConfig.ICON_REPLACE_NONE,
            NebulaConfig.ICON_REPLACE_SOLAR,
            NebulaConfig.ICON_REPLACE_LIQUID_GLASS,
            NebulaConfig.ICON_REPLACE_PLUMPY,
    };
    ...
    private int nmPendingSelection = NebulaConfig.ICON_REPLACE_NONE;
    ...
    private final Runnable nmApplyPackRunnable = () -> {
        ...
        if (generation == nmApplyGeneration
                && selection == NebulaConfig.iconReplacement
    ...
    private static boolean isRowSelected(int rowValue) {
        int stored = NebulaConfig.iconReplacement;
        if (!isKnownValue(stored)) {
            stored = NebulaConfig.ICON_REPLACE_NONE;
        }
        return rowValue == stored;
    }

    private void select(int value) {
        if (NebulaConfig.iconReplacement != value) {
            NebulaConfig.setIconReplacement(value);
    ...
    private static BaseIconReplace replaceFor(int value) {
        switch (value) {
            case NebulaConfig.ICON_REPLACE_SOLAR:        return new SolarIconReplace();
            case NebulaConfig.ICON_REPLACE_LIQUID_GLASS: return new LiquidGlassFullReplace();
            case NebulaConfig.ICON_REPLACE_PLUMPY:       return new PlumpyFullReplace();
            default:                                       return null;
        }
    }
