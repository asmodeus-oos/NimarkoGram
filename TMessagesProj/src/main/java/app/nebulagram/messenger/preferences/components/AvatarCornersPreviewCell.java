...
package app.nebulagram.messenger.preferences.components;
...
public class AvatarCornersPreviewCell extends FrameLayout {
    ...
    private final int min = (int) NebulaConfig.AVATAR_CORNERS_MIN;
    private final int max = (int) NebulaConfig.AVATAR_CORNERS_MAX;
    ...
    public AvatarCornersPreviewCell(Context context, BaseFragment fragment) {
        ...
        seekBar.setDelegate(new SeekBarView.SeekBarViewDelegate() {
            ...
            public void onSeekBarDrag(boolean stop, float progress) {
                ...
                NebulaConfig.setAvatarCorners(value);
            ...
            @Override public CharSequence getContentDescription() {
                return getString(R.string.NM_AvatarCorners) + ": " + Math.round(NebulaConfig.avatarCorners);
            }
        ...
        float progress = (NebulaConfig.avatarCorners - min) / (float) (max - min);
        seekBar.setProgress(LocaleController.isRTL ? 1f - progress : progress);
        onValueChanged(Math.round(NebulaConfig.avatarCorners), false);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float r = NebulaConfig.getAvatarCorners(56);
        ...
        if (!NebulaConfig.disableDividers && needDivider) {
