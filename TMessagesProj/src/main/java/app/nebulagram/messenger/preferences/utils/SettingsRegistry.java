package app.nebulagram.messenger.preferences.utils;
...
public class SettingsRegistry {
    ...
    public void createEntriesIfNeeded() {
        ...
        synchronized (this) {
            ...
            addBuiltIn("nebula_general", MainPreferencesActivity.ID_GENERAL, R.string.NM_Cat_General);
            addBuiltIn("nebula_appearance", MainPreferencesActivity.ID_APPEARANCE, R.string.NM_Cat_Appearance);
            addBuiltIn("nebula_chats", MainPreferencesActivity.ID_CHATS, R.string.NM_Cat_Chats);
            addBuiltIn("nebula_camera", MainPreferencesActivity.ID_CAMERA, R.string.NM_Cat_Camera);
            addBuiltIn("nebula_privacy", MainPreferencesActivity.ID_PRIVACY, R.string.NM_Cat_Privacy);
            if (PluginsController.isPluginEngineSupported()) {
                addBuiltIn("nebula_plugins", MainPreferencesActivity.ID_PLUGINS, R.string.Plugins);
            }
            addBuiltIn("nebula_media", MainPreferencesActivity.ID_NEBULA_MEDIA, R.string.NM_DownloadMedia);
            addBuiltIn("nebula_banners", MainPreferencesActivity.ID_BANNERS, R.string.NM_BAN_Title);
            addBuiltIn("nebula_wsbypass", MainPreferencesActivity.ID_WSBYPASS, R.string.NM_WSB_Title);
            addBuiltIn("nebula_textanim", MainPreferencesActivity.ID_TEXTANIM, R.string.NM_TA_Title);
            addBuiltIn("nebula_infocards", MainPreferencesActivity.ID_PILLSTACK, R.string.NM_CARDS_Title);
            addBuiltIn("nebula_debug", MainPreferencesActivity.ID_DEBUG, R.string.NM_HUB_Debug);
            addBuiltIn("nebula_restart", MainPreferencesActivity.ID_RESTART, R.string.NM_HUB_Restart);
            addBuiltIn("nebula_updates", MainPreferencesActivity.ID_UPDATES, R.string.UP_CheckForUpdates);
            entriesCreated = true;
        }
    }
    ...
    public synchronized void addLinkAliasForOption(String alias, BaseFragment activity, UItem item) {
        try {
            ...
            FileLog.e("nebula: SettingsRegistry.addLinkAliasForOption failed", t);
        }
    }
    ...
    public synchronized void addSearchEntry(BaseFragment activity, UItem item) {
        try {
            ...
            FileLog.e("nebula: SettingsRegistry.addSearchEntry failed", t);
        }
    }
    ...
    public String getFirstSettingLink(Class<?> fragmentClass, UItem item) {
        try {
            ...
            FileLog.e("nebula: SettingsRegistry.getFirstSettingLink failed", t);
        }
        return null;
    }

    public void onSettingNotFound(BaseFragment activity) {
        FileLog.d("nebula: SettingsRegistry.onSettingNotFound (no matching setting)");
    }
    ...
    private void addBuiltIn(String alias, int itemId, int titleRes) {
        ...
        Entry entry = new Entry(previous == null ? nextGuid() : previous.guid, alias,
                ...
                LocaleController.getString(R.string.NebulaGramSettings));
        entry.itemId = itemId;
        entry.explicitAlias = true;
        aliases.put(alias, entry);
        prepared.put(entry.guid, entry);
