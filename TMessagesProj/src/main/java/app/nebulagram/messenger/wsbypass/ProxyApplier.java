...
public final class ProxyApplier {
    ...
    private static synchronized void captureSnapshotIfMissing(String localHost) {
        ...
        try {
            synchronized (PROXY_LIST_LOCK) {
                ...
                if (curr != null && localHost != null && localHost.equals(host) && curr.port == NebulaWsBypassConfig.localPort) {
    ...
    private static boolean isSystemVpnActive() {
        try {
            return NebulaVpnDetector.isVpnActiveFresh();
        } catch (Throwable ignored) {
            return false;
        }
    }
    ...
    private static synchronized boolean restoreSnapshot() {
        ...
        if (NebulaWsBypassConfig.suspendOnVpn && isSystemVpnActive()) {
    ...
    public static synchronized boolean apply(int port, boolean enable, String secret, String localHost) {
        try {
            ...
            final int ownPort = port > 0 ? port : NebulaWsBypassConfig.localPort;
            final String sec = secret == null ? "" : secret.trim();
            
            final String user = "";
            final String pass = "";

            if (enable && NebulaWsBypassConfig.suspendOnVpn && isSystemVpnActive()) {
                return false;
            }
    ...
    public static synchronized void forceClearCurrent(String localHost) {
        try {
            ...
            synchronized (PROXY_LIST_LOCK) {
                ...
                if (host.equals(addr) && curr.port == NebulaWsBypassConfig.localPort) {
                    SharedConfig.currentProxy = null;
        ...
        try {
            ...
            synchronized (PROXY_LIST_LOCK) {
                ...
                for (int i = 0; i < snap.size(); i++) {
                    SharedConfig.ProxyInfo p = snap.get(i);
                    if (p == null) continue;
                    if (host.equals(p.address) && p.port == NebulaWsBypassConfig.localPort) {
                        toRemove.add(p);
                    }
                }
