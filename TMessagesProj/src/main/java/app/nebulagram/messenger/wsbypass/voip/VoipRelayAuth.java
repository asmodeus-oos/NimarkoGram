package app.nebulagram.messenger.wsbypass.voip;
...
public final class VoipRelayAuth {
    ...
    private static final String TAG = "NebulaVoIP";
    ...
    private static NebulaInlineAuth.Backend backendFor(final long uid) {
        return new NebulaInlineAuth.Backend() {
            @Override public String cachedToken() {
                String t = NebulaConfig.getVoipRelayTokenForUid(uid);
                return (t == null || t.isEmpty()) ? null : t;
            }
            @Override public void cacheToken(String t) { NebulaConfig.setVoipRelayTokenForUid(uid, t); }
            @Override public NebulaInlineAuth.Reg register(long ignored) { return httpRegister(uid); }
    ...
    public static void prefetchAsync(final int account) {
        ...
        app.nebulagram.messenger.wsbypass.RelayRegion.invalidate();
    ...
    private static Credential get(int account, boolean forceRefresh) {
        ...
        final NebulaInlineAuth.EnablePredicate permit = new NebulaInlineAuth.EnablePredicate() {
        ...
        synchronized (lock) {
            ...
            try {
                ...
                NebulaInlineAuth.Backend backend = backendFor(uid);
                if (!isAuthAllowed()) {
                    return uidOf(account) == ownerUid && isUsable(old, now) ? old : null;
                }
                String token = NebulaInlineAuth.ensureToken(
                ...
                if (fresh == null && (status[0] == 401 || status[0] == 403)) {
                    ...
                    if (!permit.runIfEnabled(() -> {
                        ...
                        if (UserConfig.getInstance(account).getClientUserId() == uid
                            ...
                            NebulaConfig.setVoipRelayTokenForUid(uid, null);
    ...
    private static NebulaInlineAuth.Reg httpRegister(long uid) {
    ...
    private static String httpPoll(long uid, String code) {
        long generation = authGeneration.get();
        if (!isAuthAllowed() || authRemainingMs() <= 0) return NebulaInlineAuth.GIVE_UP;
        HttpURLConnection con = null;
        try {
            URL url = new URL(API_URL + "/api/v1/voip/poll?user_id=" + uid
                    + "&code=" + URLEncoder.encode(code, "UTF-8"));
            con = (HttpURLConnection) url.openConnection();
            if (!registerConnection(con, generation)) return NebulaInlineAuth.GIVE_UP;
            con.setRequestMethod("POST");
            configureTimeouts(con);
            con.setDoOutput(true);
            con.getOutputStream().close();
    ...
    private static long reconcileAccountLocked(int account) {
        try {
            android.content.SharedPreferences prefs = app.nebulagram.messenger.wsbypass.NebulaWsBypassConfig.prefs();
            ...
            if (priorUid != currentUid) {
                ...
                if (priorUid > 0) {
                    ...
                    NebulaConfig.setVoipRelayTokenForUid(priorUid, null);
                }
        ...
        synchronized (authGenerationLock) {
            ...
            try {
                android.content.SharedPreferences.Editor editor =
                        app.nebulagram.messenger.wsbypass.NebulaWsBypassConfig.prefs().edit()
    ...
    private static void saveCredentialToDiskLocked(int account, Credential c) {
        try {
            android.content.SharedPreferences p = app.nebulagram.messenger.wsbypass.NebulaWsBypassConfig.prefs();
            long uid = reconcileAccountLocked(account);
