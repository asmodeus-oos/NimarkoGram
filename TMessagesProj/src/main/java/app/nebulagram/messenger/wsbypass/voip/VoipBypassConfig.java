...
public final class VoipBypassConfig {
    ...
    public static final String RELAY_HOST = "calls.nebula.org";
    public static final String ASIA_RELAY_HOST = configuredHost(
            org.telegram.messenger.BuildConfig.NEBULA_ASIA_RELAY_HOST, RELAY_HOST);
    ...
    public static String relayHost(int account) {
        return app.nebulagram.messenger.wsbypass.RelayRegion.isAsia(account) ? ASIA_RELAY_HOST : RELAY_HOST;
    }

    public static String[] relayHosts(int account) {
        if (app.nebulagram.messenger.wsbypass.RelayRegion.isAsia(account)) {
            return new String[]{ASIA_RELAY_HOST, RELAY_HOST};
        }
        return new String[]{RELAY_HOST, ASIA_RELAY_HOST};
    }

    public static final String VOIP_AUTH_URL = "https://calls.nebula.org";
    ...
    public static boolean isDataBypassEnabled() {
        synchronized (RELAY_STATE_LOCK) {
            return NebulaWsBypassConfig.enabled;
        }
    }

    public static boolean isSuspendOnVpnEnabled() {
        synchronized (RELAY_STATE_LOCK) {
            return NebulaWsBypassConfig.suspendOnVpn;
        }
    }
    ...
    private static boolean isVoipBypassEnabledLocked() {
        SharedPreferences p = NebulaWsBypassConfig.prefs();
    ...
    public static void setVoipBypassEnabled(boolean v) {
        mutateRelayState(() -> {
            SharedPreferences preferences = NebulaWsBypassConfig.prefs();
    ...
    private static boolean dataBypassRequiredLocked(boolean freshVpnState) {
        return NebulaWsBypassConfig.enabled && !vpnSuspendsRelayLocked(freshVpnState);
    }

    private static boolean vpnSuspendsRelayLocked(boolean freshVpnState) {
        if (!NebulaWsBypassConfig.suspendOnVpn) return false;
        return freshVpnState
                ? app.nebulagram.messenger.wsbypass.NebulaVpnDetector.isVpnActiveFresh()
                : app.nebulagram.messenger.wsbypass.NebulaVpnDetector.isVpnActive();
    }
