package app.nebulagram.messenger.wsbypass;
...
public final class NebulaWsBypassController {
    ...
    private static volatile NebulaWsBypassController instance;

    public static NebulaWsBypassController getInstance() {
        NebulaWsBypassController local = instance;
        if (local == null) {
            synchronized (NebulaWsBypassController.class) {
                local = instance;
                if (local == null) {
                    local = new NebulaWsBypassController();
                    instance = local;
                }
            }
        }
    ...
    private NebulaWsBypassController() {
    }
    ...
    public boolean blockedByVpn() {
        return app.nebulagram.messenger.wsbypass.voip.VoipBypassConfig
                .isSuspendOnVpnEnabled() && NebulaVpnDetector.isVpnActive();
    }

    private boolean blockedByVpnFresh() {
        return app.nebulagram.messenger.wsbypass.voip.VoipBypassConfig
                .isSuspendOnVpnEnabled() && NebulaVpnDetector.isVpnActiveFresh();
    }
    ...
    public void reevaluateForVpnToggle() {
        onVpnStateChanged(NebulaVpnDetector.isVpnActiveFresh());
    }

    public String getConnectionState() {
        if (!app.nebulagram.messenger.wsbypass.voip.VoipBypassConfig
    ...
    public void ensureStartedSync() {
        ...
        try {
            ...
            FileLog.e("NebulaWsBypassController.ensureStartedSync", t);
            releaseStart(token);
        }
    }
    ...
    public void setEnabled(boolean v) {
        ...
        synchronized (lifecycleLock) {
            ...
            NebulaWsBypassConfig.setEnabled(v);
            if (v) {
    ...
    public void onVpnStateChanged(boolean vpnActive) {
        ...
        boolean suspend = vpnActive
                && app.nebulagram.messenger.wsbypass.voip.VoipBypassConfig
    ...
    public void stop() {
        synchronized (lifecycleLock) {
            ...
            try {
                ...
                FileLog.e("NebulaWsBypassController.stop core error", t);
            }
            try {
                ProxyApplier.apply(0, false, "", WsBypassCore.LOCAL_PROXY_HOST);
            } catch (Throwable t) {
                FileLog.e("NebulaWsBypassController.stop proxy error", t);
            }
            try {
    ...
    public void onAppResume() {
        final boolean dataEnabled = app.nebulagram.messenger.wsbypass.voip.VoipBypassConfig
                .isDataBypassEnabled();
        final boolean suspendOnVpn = app.nebulagram.messenger.wsbypass.voip.VoipBypassConfig
                .isSuspendOnVpnEnabled();
    ...
    private void launchStartThread(final long token) {
        ...
        try {
            ...
            FileLog.e("NebulaWsBypassController start thread failed", startFailure);
            notifyReloader();
        }
    }
    ...
    private void startSync(long token) {
        synchronized (lifecycleLock) {
            ...
            try {
                ...
                int desiredPort = NebulaWsBypassConfig.localPort;
                String secret = NebulaWsBypassConfig.mtprotoSecret;
                if (secret == null || secret.isEmpty()) {
                    secret = MtprotoHandshake.generateSecretHex();
                    NebulaWsBypassConfig.setMtprotoSecret(secret);
                }
                ...
                if (boundPort > 0 && boundPort != desiredPort) {
                    NebulaWsBypassConfig.setLocalPort(boundPort);
                }
                ...
                try {
                    ...
                    FileLog.e("NebulaWsBypassController.startSync proxy apply error", t);
                ...
                try {
                    ...
                    if (app.nebulagram.messenger.wsbypass.voip.VoipBypassConfig.isVoipBypassEnabled()) {
                        app.nebulagram.messenger.wsbypass.voip.VoipRelayAuth.prefetchAsync(account);
                    }
                } catch (Throwable ignored) {}
                ensureWatchdogLocked();
                FileLog.d("NebulaWsBypassController: started on 127.0.0.1:" + boundPort);
            } catch (Throwable t) {
                FileLog.e("NebulaWsBypassController.startSync error", t);
                if (blockedByVpnFresh()) {
                    lastStartFailed = false;
                    lastError = "";
                    suspendForVpn();
    ...
    private void startWatchdogLocked() {
        ...
        try {
            watchdogTask = pool.scheduleAtFixedRate(new Runnable() {
                ...
                public void run() {
                    try {
                        ...
                        FileLog.e("NebulaWsBypassController watchdog tick error", t);
                    }
                }
            ...
            watchdogPool = null;
            pool.shutdownNow();
            FileLog.e("NebulaWsBypassController watchdog start failed", scheduleFailure);
        }
    }
    ...
    private void watchdogTick(long generation) {
        synchronized (lifecycleLock) {
            ...
            if (!running) {
                FileLog.d("NebulaWsBypassController watchdog: enabled but not running, restarting");
                restartCoreLocked();
                return;
            }
            ...
            if (!coreAlive) {
                FileLog.d("NebulaWsBypassController watchdog: core dead while running, restarting");
                restartCoreLocked();
                return;
            }
            ...
            if (!ProxyApplier.isLocalProxyActive(
                ...
                FileLog.d("NebulaWsBypassController watchdog: proxy state diverged, re-applying");
    ...
    private void restartCoreLocked() {
        ...
        try {
            ...
            FileLog.e("NebulaWsBypassController.restartCore error", t);
        }
    }
    ...
    private static void cancelRelayAuthConnections() {
        ...
        app.nebulagram.messenger.wsbypass.voip.VoipRelayAuth.cancelPendingAuth();
    }
