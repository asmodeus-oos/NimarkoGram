...
public final class WsRelayAuth {
    ...
    private static final String TAG = "NebulaKWS";
    private static final String API_URL = "https://calls.nebula.org";
    ...
    private static NebulaInlineAuth.Backend backendFor(final long uid) {
        return new NebulaInlineAuth.Backend() {
            @Override public String cachedToken() {
                String t = NebulaConfig.getWsRelayTokenForUid(uid);
                return (t == null || t.isEmpty()) ? null : t;
            }
            @Override public void cacheToken(String t) { NebulaConfig.setWsRelayTokenForUid(uid, t); }
            @Override public NebulaInlineAuth.Reg register(long u) { return httpRegister(u); }
    ...
    private static long reconcileAccountLocked(int account) {
        try {
            android.content.SharedPreferences prefs = NebulaWsBypassConfig.prefs();
            ...
            if (priorUid != currentUid) {
                ...
                if (priorUid > 0) {
                    ...
                    NebulaConfig.setWsRelayTokenForUid(priorUid, null);
                }
    ...
    public static void onAccountLoggedOut(int account, long uid) {
        synchronized (authGenerationLock) {
            ...
            cached.remove(account);
            try {
                android.content.SharedPreferences.Editor editor = NebulaWsBypassConfig.prefs().edit()
                        .remove(PREF_SLOT_UID + account).remove(PREF_CRED);
                if (uid > 0) {
                    editor.remove(credentialKey(uid));
                    NebulaConfig.setWsRelayTokenForUid(uid, null);
                }
                editor.apply();
            } catch (Throwable ignore) {}
        }
    ...
    private static void saveCredentialToDiskLocked(int account, Credential c) {
        try {
            android.content.SharedPreferences p = NebulaWsBypassConfig.prefs();
            long uid = reconcileAccountLocked(account);
