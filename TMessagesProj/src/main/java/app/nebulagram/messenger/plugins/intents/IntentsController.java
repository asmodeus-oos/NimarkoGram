package app.nebulagram.messenger.plugins.intents;
...
public class IntentsController {
    ...
    public String addIntentHook(PyObject filterDict) {
        ...
        if (!committed) {
            FileLog.w("nebula intents: rejected late handler for inactive plugin " + rec.pluginId);
            return null;
        }
        FileLog.d("nebula intents: registered handler " + rec.handlerId + " for plugin " + rec.pluginId);
        return result[0];
    }
    ...
    public void removeIntentHooksByPluginId(
        ...
        if (removed > 0) {
            FileLog.d("nebula intents: removed " + removed
    ...
    public boolean dispatchIntent(Intent intent) {
        ...
        try {
            ...
            FileLog.e("nebula intents: dispatch failed", t);
            return false;
        }
    }
    ...
    private boolean isPluginActive(String pluginId) {
        try {
            app.nebulagram.messenger.plugins.Plugin p =
