...
public class MenuItemRecord {
    ...
    public MenuItemRecord(String pluginId, PyObject pyObject) {
        ...
        if (!TextUtils.isEmpty(this.conditionString)) {
            try {
                ...
                try {
                    org.telegram.messenger.FileLog.d("nebula: invalid menu condition for plugin "
        ...
        if (!TextUtils.isEmpty(this.iconName)) {
            ...
            if (resId == 0) {
                try {
                    org.telegram.messenger.FileLog.d("nebula: plugin menu icon '" + this.iconName + "' not found in drawables");
                } catch (Throwable ignored) {}
            }
        }
    ...
    public boolean checkCondition(Map<String, Object> map) {
        ...
        try {
            ...
            try {
                org.telegram.messenger.FileLog.d("nebula: MenuItemRecord.checkCondition failed for plugin "
                        + this.pluginId + " condition='" + this.conditionString + "': " + e);
            } catch (Throwable ignored) {}
            return false;
        }
