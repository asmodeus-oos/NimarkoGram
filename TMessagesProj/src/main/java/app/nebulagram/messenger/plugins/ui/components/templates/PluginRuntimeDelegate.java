package app.nebulagram.messenger.plugins.ui.components.templates;
    ...
    public static boolean run(
        ...
        try {
            ...
            controller.getWatchdog().onPluginExecutionFailed(
                    pluginId, error);
            FileLog.e("NebulaGram: plugin UI callback failed for "
                    + pluginId, error);
            return false;
        } finally {
    ...
    static boolean runScoped(
        ...
        boolean invoked = run(token, () -> {
            try {
                ...
                FileLog.e("NebulaGram: plugin traversal callback failed for "
                        + token.getPluginId(), error);
            } finally {
