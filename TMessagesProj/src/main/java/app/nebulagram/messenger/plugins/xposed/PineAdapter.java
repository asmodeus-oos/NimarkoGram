package app.nebulagram.messenger.plugins.xposed;
...
import app.nebulagram.messenger.plugins.bridge.PythonBoundarySanitizer;
...
public final class PineAdapter extends MethodHook {
    ...
    static {
        ...
        try {
            ...
            FileLog.e("nebula: PineAdapter reflection init failed", t);
        }
    ...
    public void beforeCall(Pine.CallFrame cf) throws Throwable {
        if (cf == null) {
            FileLog.e("nebula: Pine supplied a null CallFrame for " + member);
            return;
        }
        ...
        if (receiverMismatched(cf)) {
            FileLog.w("nebula: Pine type-mismatch on " + member
        ...
        try {
            try {
                ...
                FileLog.e("nebula: hook callback threw for " + member, t);
                callbackFailed = true;
            ...
            if (!callbackFailed) {
                ...
                if (invocationContainsRawPythonProxy(cf)) {
                    FileLog.w("nebula: raw Python proxy in hook invocation state on "
            ...
            if (param.hasThrowable()) {
                ...
                if (result == INVALID_RESULT) {
                    FileLog.w("nebula: incompatible hook result on " + member
                ...
                FileLog.w("nebula: Pine arg type-mismatch on " + member
                        + " from a plugin hook — restoring original arguments");
                param.restoreInvocation(cf);
            } else if (receiverMismatched(cf)) {
                FileLog.w("nebula: Pine receiver type-mismatch on " + member
    ...
    public void afterCall(Pine.CallFrame cf) throws Throwable {
        ...
        if (param == null) {
            FileLog.w("nebula: missing before-hook state for " + member);
            param = claim(cf);
        }
        try {
            ...
            try {
                ...
                FileLog.e("nebula: afterHookedMethod threw", t);
                callbackFailed = true;
                param.restoreInvocation(cf);
            ...
            if (!callbackFailed) {
                ...
                if (receiverMismatched(cf) || argsTypeMismatched(cf)) {
                    FileLog.w("nebula: incompatible after-hook invocation state on "
                            + member + " — restoring original receiver/arguments");
                    param.restoreInvocation(cf);
                }
            }
            if (param.hasThrowable()) {
                ...
                Object result = normalizeResult(member, param.getResult());
                if (result == INVALID_RESULT) {
                    FileLog.w("nebula: incompatible after-hook result on " + member
