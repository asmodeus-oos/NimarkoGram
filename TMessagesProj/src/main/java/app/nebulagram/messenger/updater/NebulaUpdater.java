...
public class NebulaUpdater {
    ...
    private static final String ENDPOINT = "https://update.ettacent.dev/api/app-version?id=nebulagram";

    public static final DispatchQueue otaQueue = new DispatchQueue("nebulaOtaQueue");
    ...
    private static String wantedApkHash() {
        ...
        if (wantHash == null || wantHash.isEmpty()) wantHash = NebulaUpdateConfig.getApkSha256();
    ...
    public static Update getOrRestoreLastUpdate() {
        ...
        String v = NebulaUpdateConfig.getLastUpdateVersion();
        String url = NebulaUpdateConfig.getLastUpdateUrl();
        if (v == null || v.isEmpty() || url == null || url.isEmpty()) return null;
        lastUpdate = new Update(
                v,
                NebulaUpdateConfig.getLastUpdateVersionCode(),
                NebulaUpdateConfig.getLastUpdateChangelog(),
                NebulaUpdateConfig.getLastUpdateSize(),
                url,
                NebulaUpdateConfig.getLastUpdateDate());
        return lastUpdate;
    }
    ...
    public static boolean checkDirs() {
        try {
            ...
            if (version == null || version.isEmpty()) version = NebulaUpdateConfig.getUpdateVersionName();
            ...
            if (Looper.myLooper() == Looper.getMainLooper()) {
                ...
                if (apkFile.exists() && !updateDownloaded) otaQueue.postRunnable(NebulaUpdater::checkDirs);
                return true;
            }
    ...
    public static void checkOnLaunch(BaseFragment fragment) {
        if (launchChecked || !NebulaUpdateConfig.getAutoOTA()) return;
        launchChecked = true;
    ...
    public static void checkUpdates(BaseFragment fragment, boolean manual, OnUpdateNotFound onUpdateNotFound, Runnable onUpdateFound, OnCheckFailed onCheckFailed) {
        long lastThrottle = NebulaUpdateConfig.getUpdateScheduleTimestamp();
        ...
        otaQueue.postRunnable(() -> {
            NebulaUpdateConfig.setLastUpdateCheckTime(System.currentTimeMillis());
            try {
                HttpURLConnection connection = (HttpURLConnection) new URI(ENDPOINT).toURL().openConnection();
                connection.setRequestMethod("GET");
                connection.setRequestProperty("User-Agent", "NebulaGram-OTA");
                ...
                if (version == null || !VERSION_PATTERN.matcher(version).matches() || version.equals(".") || version.equals("..")) {
                    FileLog.e("NebulaUpdater: rejecting unsafe version from server: " + version);
                    version = "";
                ...
                if (!isHttps(downloadURL)) {
                    FileLog.e("NebulaUpdater: rejecting non-https download url: " + downloadURL);
                    version = "";
                    downloadURL = "";
                    if (onUpdateNotFound != null) AndroidUtilities.runOnUIThread(onUpdateNotFound::run);
                    checkingForUpdates = false;
                ...
                lastUpdate = update;
                NebulaUpdateConfig.setLastUpdate(version, versionCode, downloadURL, changelog, size, uploadDate);
                if (update.isNew() && fragment != null && fragment.getContext() != null) {
                    checkDirs();
                    AndroidUtilities.runOnUIThread(() -> {
                        NebulaUpdaterSheet.showAlert(fragment, true, update);
                        if (onUpdateFound != null) onUpdateFound.run();
                    });
                    NebulaUpdateConfig.setUpdateIsDownloading(false);
                    NebulaUpdateConfig.setUpdateAvailable(true);
                    if (version != null && !version.isEmpty()) NebulaUpdateConfig.setUpdateVersionName(version);
                    NebulaUpdateConfig.setUpdateSize(size);
                } else {
                    if (onUpdateNotFound != null) AndroidUtilities.runOnUIThread(onUpdateNotFound::run);
                    cleanOtaDir();
                }
    ...
    private static boolean hasPersistedDownloadLocked() {
        if (!NebulaUpdateConfig.getUpdateIsDownloading()) return false;
        String link = NebulaUpdateConfig.getPausedDownloadLink();
        String targetVersion = NebulaUpdateConfig.getPausedDownloadVersion();
    ...
    public static DownloadUiState getDownloadUiState() {
        synchronized (downloadBindingLock) {
            ...
            if (progress <= 0) {
                progress = Math.max(0, Math.round(NebulaUpdateConfig.getUpdateDownloadingProgress()));
            }
    ...
    public static void downloadApk(Context context, String link, String title, long ownerToken) {
        ...
        synchronized (downloadBindingLock) {
            ...
            dlRealProgress = 0;
            dlShownProgress = 0;
            NebulaUpdateConfig.setPausedDownloadLink(link);
            NebulaUpdateConfig.setPausedDownloadVersion(version);
            NebulaUpdateConfig.setPausedDownloadOffset(0);
            NebulaUpdateConfig.setPausedDownloadSize(expectedSizeBytes);
            NebulaUpdateConfig.setPausedDownloadSha256(expectedSha256);
            NebulaUpdateConfig.setUpdateIsDownloading(true);
        ...
        otaQueue.postRunnable(() -> {
            ...
            try {
                ...
                try (InputStream in = connection.getInputStream();
                    ...
                    while ((read = in.read(buf)) != -1) {
                        ...
                        if (downloadPaused) {
                            ...
                            out.flush();
                            out.getFD().sync();
                            NebulaUpdateConfig.setPausedDownloadOffset(downloaded);
                        ...
                        if (expectedTotal > 0) {
                            ...
                            NebulaUpdateConfig.setUpdateDownloadingProgress(dlRealProgress);
                            NebulaUpdateConfig.setPausedDownloadOffset(downloaded);
                ...
                if (expectedSha256 != null && !expectedSha256.isEmpty()) {
                    ...
                    hashVerified = true;
                    NebulaUpdateConfig.setApkSha256(expectedSha256);
                } else {
                    NebulaUpdateConfig.setApkSha256(null);
                }
                ...
                synchronized (downloadBindingLock) {
                    ...
                    activeDownloadGeneration = -1;
                    NebulaUpdateConfig.setUpdateIsDownloading(false);
                    NebulaUpdateConfig.clearPausedDownload();
                }
                ...
                if (downloadCanceled || downloadPaused || myGeneration != downloadGeneration) {
                    if (downloadPaused && !downloadCanceled && myGeneration == downloadGeneration) {
                        try {
                            ...
                            pausedBytes = onDisk;
                            NebulaUpdateConfig.setPausedDownloadOffset(onDisk);
    ...
    private static HttpURLConnection openApkConnection(String link, long rangeStart) throws Exception {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            c.setRequestProperty("User-Agent", "NebulaGram-OTA");
    ...
    private static final String UPDATE_CHANNEL = "nebula_updates";
    ...
    private static PendingIntent pauseResumeIntent(Context context, boolean pause) {
        Intent i = new Intent(context, NebulaUpdaterReceiver.class);
        i.setAction(pause ? NebulaUpdaterReceiver.ACTION_PAUSE : NebulaUpdaterReceiver.ACTION_RESUME);
    ...
    private static PendingIntent openUpdateScreenIntent(Context context) {
        ...
        open.setAction("app.nebulagram.messenger.OPEN_UPDATE");
    ...
    private static PendingIntent cancelDownloadIntent(Context context) {
        Intent cancel = new Intent(context, NebulaUpdaterReceiver.class);
        cancel.setAction(NebulaUpdaterReceiver.ACTION_CANCEL);
    ...
    public static void cancelDownload(Context context, long downloadId) {
        ...
        synchronized (downloadBindingLock) {
            ...
            cancelUpdateNotification();
            NebulaUpdateConfig.setUpdateIsDownloading(false);
            NebulaUpdateConfig.setUpdateDownloadingProgress(0f);
            NebulaUpdateConfig.clearPausedDownload();
        }
        if (c != null) {
    ...
    public static void resumeDownload(Context context) {
        ...
        synchronized (downloadBindingLock) {
            if (downloadLink == null || (version == null || version.isEmpty())) {
                String savedLink = NebulaUpdateConfig.getPausedDownloadLink();
                String savedVersion = NebulaUpdateConfig.getPausedDownloadVersion();
            ...
            if (expectedSizeBytes <= 0) {
                long savedSize = NebulaUpdateConfig.getPausedDownloadSize();
                if (savedSize > 0) expectedSizeBytes = savedSize;
            }
            if (expectedSha256 == null || expectedSha256.isEmpty()) {
                String savedHash = NebulaUpdateConfig.getPausedDownloadSha256();
    ...
    public static class Update {
        ...
        public boolean isNew() {
            ...
            NebulaUpdateConfig.setUpdateAvailable(isNew);
            return isNew;
        }
    }
}
